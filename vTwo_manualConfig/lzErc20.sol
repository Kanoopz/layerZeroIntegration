...
contract lzErc20 is NonblockingLzApp, IERC20, IERC20Metadata 
{
    ...
    */
    ...
    function crossChainTransfer(uint16 _dstChainId, bytes memory _payload, bytes memory _adapterParams) public payable
    {

        _lzSend(_dstChainId, _payload, payable(tx.origin), address(0x0), _adapterParams, msg.value);   
    }
    
    function getBasicAdapter() public pure returns(bytes memory)
    {
        uint16 valueOne = 1;
        uint256 valueTwo = 200000;

        return abi.encodePacked(valueOne, valueTwo);
    }

    function getCrossChainaValueToSend(uint256 paramEstimatedFee) public pure returns(bytes memory)
    {
        uint16 valueOne = 1;
        uint256 valueTwo = paramEstimatedFee;

        return abi.encodePacked(valueOne, valueTwo);
    }

    function getPayloadToCrossChainTransfer(address paramDestinationAddressToTransfer, uint256 paramQuantityToTransfer) public pure returns(bytes memory)
    {
        return abi.encode(paramDestinationAddressToTransfer, paramQuantityToTransfer);
    }
